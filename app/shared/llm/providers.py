...
def _validate_endpoint(url: str, *, allow_http: bool = False) -> None:
    ...
    if not hostname:
        raise LLMCallError(LLMCallError.KIND_BAD_REQUEST, "endpoint_url has no hostname")
    try:
        infos = socket.getaddrinfo(hostname, None, proto=socket.IPPROTO_TCP)
    except socket.gaierror:
        raise LLMCallError(LLMCallError.KIND_BAD_REQUEST, f"Cannot resolve hostname: {hostname}")
    for info in infos:
        ip = ipaddress.ip_address(info[4][0])
        if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved:
            raise LLMCallError(
                LLMCallError.KIND_BAD_REQUEST,
                "LLM endpoint must not resolve to a private or reserved IP address",
            )
