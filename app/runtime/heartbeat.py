...
from app.runtime.install_state import get_install_id

logger = logging.getLogger(__name__)

HEARTBEAT_ENDPOINT = os.getenv(
    "ZNYX_HEARTBEAT_URL",
    # Default target is the ZNYX prod Control Plane (receiver lives at
    # app/control_plane/api/install_telemetry_routes.py). Override with
    # ZNYX_HEARTBEAT_URL for preprod (https://pprd-cp.znyx.ai/...),
    # self-hosted CPs, or local testing (http://localhost:8001/v1/install-telemetry).
...
class Heartbeat:
    ...
    async def _send_ping(self, event_type: str = "heartbeat", run_count: int = 0) -> None:
        ...
        try:
            import httpx
            from app.runtime.install_state import get_run_count
