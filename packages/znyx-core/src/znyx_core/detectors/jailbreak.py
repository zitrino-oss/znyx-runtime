...
class JailbreakDetector:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._dangerous_roleplay_re = re.compile(
            ...
            # ignore/bypass/disable/override/circumvent/disregard (any inflection) the
            # model's safety/rules/guidelines — allow an intervening qualifier
            # (your/its/the/all/any) so "ignore all safety restrictions",
            # "bypasses its safety guidelines" and "ignores safety" are caught, not just
            # the bare "ignore your rules" form.
            r'(?:ignore|ignores|ignoring|bypass|bypasses|bypassing|disable|disables|disabling|'
            r'override|overrides|overriding|circumvent|circumvents|disregard|disregards)\s+'
            r'(?:(?:your|its|their|the|all|any)\s+)?(?:safety\s+)?'
            r'(?:rules?|restrictions?|guidelines?|safety|limits?|filters?|constraints?|protocols?|safeguards?)|'
